...
def read_all_stock_files(individual_stocks_5yr):
    allFiles = []
    for (_, _, files) in os.walk('C:/Stock_Project/Stock-Prices-Prediction-ML-Flask/individual_stocks_5yr_2/.'):
        allFiles.extend(files)
        break

    dataframe_dict = {}
    for stock_file in allFiles:
        df = pd.read_csv('C:/Stock_Project/Stock-Prices-Prediction-ML-Flask/individual_stocks_5yr_2/.' + "/" +stock_file)
...
def SVR_linear(dates, prices, test_date, df):
    ...
    X_train, X_test, y_train, y_test = train_test_split(trainX, trainY, test_size = 0.10, random_state =43)
